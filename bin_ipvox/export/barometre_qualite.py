...
class BarometreQuality(base_csv_export.BaseExport):
    ...
    def va_exporter(self, dict , *args, **kwargs):
        ...
        # generation
        td    = date.today()
        fdst  = '%s/export_barometre_qualite_%s%s%s.csv'%(dir, td.year,
                                            td.month, td.day)      
...
def  send_mail(dir):
    body  ="""
    Bonjoura tous <br/>
    Veuillez trouver ci-joint l'export du fichier barometre qualite
    %s.<br/>
    Cordialement Informatique<br/> """%(datetime.now())
    to= [
        'adia@pcci.sn',
        'cyril.fontaine@canal-plus.com',
        'florence.rimauro@canal-overseas.com',
        'anne.laurent@canal-plus.com',
        'nicolas.niscoise@canal-plus.com',
        'melanie.martinet@canal-plus.com',
        'smgning@pcci.sn',
        'magoudiaby@pcci.sn',
        'andoye@pcci.sn'
    ]
    ...
    subject="export fichier barometre qualite  %s" %( datetime.now() )    
...
def target(lock =None , *args):
    ...
    while True :
        ...
        try:
            _s= BarometreQuality()
            _s.va_exporter(config.CONFIG_EXPORT_BAROMETRE_QUALITE)
