...
class BarometreQuality(base_csv_export.BaseExport):
    ...
    def do_query(self, qs, with_date=False):
        self.execute_execute_file_from_outband(qs)
    ...
    def va_exporter(self, dict , *args, **kwargs):
        ...
        # generation
        td    = date.today()
        fdst  = '%s/export_fiches_cloturees_%s%s%s.csv'%(dir, td.year,
                                            td.month, td.day)      
...
def  send_mail(dir):
    body  ="""
    Bonjour a tous  <br/>
    Veuillez trouver ci-joint l'export du fichier des fiches cloturees
    %s.<br/>
    ...
    subject="export des fiches cloturees %s" %( datetime.now() )    
...
def target(lock =None , *args):
    ...
    while True :
        ...
        try:
            _s= BarometreQuality()
            _s.va_exporter(config.CONFIG_EXPORT_FICHES_CLOTUREES) 
        except Exception, e:
