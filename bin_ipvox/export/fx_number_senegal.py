...
class BarometreQuality(base_csv_export.BaseExport):
    ...
    def va_exporter(self, dict , *args, **kwargs):
        ...
        # generation
        td    = date.today()
        fdst  = '%s/FX_NUMBR22013_%s_%s_%s_SEN_AS_GLOGAL_%s%s%s_Faux_Numeros.csv'\
                %(dir, td.year,
...
def  send_mail(dir):
    body  ="""
    Bonjour a tous <br/>
    Veuillez trouver ci-joint l'export du fichier faux numeros senegal du
    %s.<br/>
    ...
    """
    subject="export faux numeros senegal %s" %( datetime.now() )    
...
def target(lock =None , *args):
    ...
    while True :
        ...
        try:
            _s= BarometreQuality()
            _s.va_exporter(config.CONFIG_EXPORT_FX_NUMBERS_SENEGAL) 
        except Exception, e:
