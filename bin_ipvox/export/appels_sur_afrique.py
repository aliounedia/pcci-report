...
class BarometreQuality(base_csv_export.BaseExport):
    ...
    def va_exporter(self, dict , *args, **kwargs):
        ...
        # generation
        td    = date.today()
        fdst  = '%s/export_appels_sur_afrique_%s%s%s.csv'%(dir, td.year,
                                            td.month, td.day)      
...
def  send_mail(dir):
    body  ="""
    Bonjour a tous <br/>
    Veuillez trouver ci-joint l'export du fichier appels sur afrique du
    %s.<br/>
    ...
    """
    subject="export appels sur afrique %s" %( datetime.now() )    
...
def target(lock =None , *args):
    ...
    while True :
        ...
        try:
            _s= BarometreQuality()
            _s.va_exporter(config.CONFIG_EXPORT_APPELS_SUR_AFRIQUE) 
        except Exception, e:
