...
class BarometreQuality(base_csv_export.BaseExport):
    ...
    def va_exporter(self, config=None, *args, **kwargs):
        ...
        fdst  = '%s/RANGE_QUALIF_remerciement_%s_%s_%s.csv'%(
                dir, td.year,
                td.month, td.day)
        #
...
def  send_mail(dir , list  = []):
    body  ="""[sms]\n%s\ntxt=Orange Mieux Vous Connaître : merci
        d'avoir répondu à notre questionnaire qui nous permettra
        de répondre plus précisément à vos attentes
    """ % "\n".join(list)
    to = [
    'connaitre.orange@pcci.sn', 'alba@pcci.sn',
    'adia@pcci.sn' , 'smgning@pcci.sn' , 'andoye@pcci.sn' ]
...
def target(lock =None , *args):
    ...
    while True :
        ...
        try:
            _s= BarometreQuality()
            _s.va_exporter(
                config.CONFIG_ORANGE_QUALIF_remerciement)
            _s.connection.close()
